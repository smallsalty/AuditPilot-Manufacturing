...
| `industry_benchmark_snapshot` | 衍生行业基准 | 同行业指标对比 | 必须排除 `mock` 来源 |
...
| `FIN_DATA_INDUSTRY_DEVIATION` | 行业对比至少 2 项异常 | 2 项异常 `78`；3 项及以上 `88` |

行业对比异常定义：

| 指标 | 异常条件 |
| --- | --- |
| 毛利率高于行业 | `gap >= 8` 个百分点，或 `percentile >= 0.90`，或 `zscore >= 2.0` |
| 应收周转低于行业 | `gap_pct <= -0.30`，或 `percentile <= 0.20`，或 `zscore <= -1.5` |
| 资产负债率高于行业 | `gap >= 10` 个百分点，或 `percentile >= 0.85`，或 `zscore >= 1.5` |
...
行业基准来自 `IndustryBenchmarkSnapshot`。
