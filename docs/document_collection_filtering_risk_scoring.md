...
文档抽取优先尝试通过统一 `LLMClient` 调 DeepSeek Anthropic 兼容接口。LLM 输出失败或不可用时，不会让文档解析整体失败，而是回退到规则候选结果。
...
### 4.1 行业龙头基准使用东方财富业绩报表

行业分类直接使用东方财富业绩报表的 `所处行业` 字段。同一行业内按营业收入、归母净利润和股票代码稳定排序，最多选取 Top 5 龙头企业。

每家公司先计算指标，再取算术均值。单项指标至少需要 3 家有效样本。刷新失败或样本不足时不使用 mock，也不展示旧期间基准。

### 4.2 LLM 摘要不是风险结果可用前提

DeepSeek 摘要失败、限流、过载或返回不规范 JSON 时，系统会 fallback 到本地摘要或规则抽取结果。
...
行业对比链路不再读取 mock 行业基准 CSV。刷新状态、龙头企业和龙头均值分别写入 `industry_benchmark_refresh_state`、`industry_leader_company` 和 `industry_leader_benchmark`。
