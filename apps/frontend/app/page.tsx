    ...
    <div className="space-y-6 pb-10">
      <section className="rounded-xl border bg-card p-6 shadow-soft">
        <div className="grid gap-6 lg:grid-cols-[1.4fr_0.8fr]">
          <div>
            <p className="text-xs uppercase tracking-[0.32em] text-muted-foreground">风险总览</p>
            <h2 className="mt-3 max-w-3xl text-4xl font-semibold leading-tight text-foreground">
          ...
          <Card>
            <p className="text-xs uppercase tracking-[0.24em] text-muted-foreground">当前企业</p>
            <div className="mt-4">
              {enterpriseError ? (
                ...
                <div className="rounded-xl border border-dashed bg-muted/30 px-4 py-4 text-sm text-muted-foreground">
                  正在初始化企业上下文...
                </div>
              ) : currentEnterprise ? (
                <div className="rounded-xl border bg-muted/20 px-4 py-4">
                  <p className="text-lg font-semibold text-foreground">{currentEnterprise.name}</p>
                  ...
                  {readiness?.manual_parse_required ? (
                    <p className="mt-1 text-sm text-emerald-700">已同步，待手动解析：{readiness.documents_pending_parse} 份</p>
                  ) : null}
                  <p className="mt-1 text-sm text-muted-foreground">风险分析状态：{formatAnalysisStatus(analysisStatus)}</p>
                </div>
      ...
      <section className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <Card>
          <div className="mb-4">
            <p className="text-xs uppercase tracking-[0.24em] text-muted-foreground">风险雷达</p>
            <h3 className="mt-2 text-xl font-semibold text-foreground">风险结构画像</h3>
          </div>
          {enterpriseError ? (
            ...
            <div className="flex h-[340px] items-center justify-center rounded-xl border border-dashed bg-muted/30 px-6 text-sm text-muted-foreground">
              正在加载风险画像...
            </div>
        ...
        <Card>
            <p className="text-xs uppercase tracking-[0.24em] text-muted-foreground">风险趋势</p>
          <h3 className="mt-2 text-xl font-semibold text-foreground">最近分析趋势</h3>
          {enterpriseError ? (
            <div className="flex h-[340px] items-center justify-center rounded-2xl border border-red-400/20 bg-red-500/10 px-6 text-sm text-red-100">
              企业上下文初始化失败。
            </div>
      ...
      <section className="grid gap-6 xl:grid-cols-[1fr_1fr]">
        <Card>
          <p className="text-xs uppercase tracking-[0.24em] text-muted-foreground">高风险事项</p>
          <h3 className="mt-2 text-xl font-semibold text-foreground">Top 风险项</h3>
          {enterpriseError ? (
            ...
            <div className="mt-4 space-y-3">
              {topRisks.map((risk) => (
                <div key={risk.id} className="rounded-xl border bg-muted/20 p-4">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <p className="font-medium text-foreground">{risk.risk_name}</p>
                      <p className="mt-1 text-sm text-muted-foreground">
                        {formatRiskLevel(risk.risk_level)} | {formatSourceType(risk.source_type)}
                      </p>
                    </div>
                    <span className="text-2xl font-semibold text-primary">{risk.risk_score.toFixed(1)}</span>
                  </div>
                </div>
              ))}
            </div>
          ) : (
            <div className="mt-4 rounded-xl border border-dashed bg-muted/30 p-4 text-sm text-muted-foreground">暂无高风险事项。</div>
          )}
        </Card>

        <Card>
          <p className="text-xs uppercase tracking-[0.24em] text-muted-foreground">数据状态</p>
          <h3 className="mt-2 text-xl font-semibold text-foreground">当前数据来源与准备情况</h3>
          <div className="mt-4 grid gap-3 text-sm text-muted-foreground">
            <div className="rounded-xl border bg-muted/20 p-4">企业主数据来源：AkShare</div>
            <div className="rounded-xl border bg-muted/20 p-4">公告与处罚来源：巨潮资讯 / 上传文档</div>
