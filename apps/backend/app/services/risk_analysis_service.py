...
from app.models import AnalysisRun, AuditRecommendation, AuditRule, IndustryBenchmark, RiskAlertRecord, RiskIdentificationResult
...
class RiskAnalysisService:
    ...
    def run(self, db: Session, enterprise_id: int) -> dict:
        ...
        benchmarks = list(
            db.scalars(select(IndustryBenchmark).where(IndustryBenchmark.industry_tag == enterprise.industry_tag)).all()
        )
        run = AnalysisRun(enterprise_id=enterprise_id, status="running", trigger_source="manual")
        db.add(run)
        db.commit()
        db.refresh(run)
        ...
        for rule in rules:
            ...
            db.add(
                AuditRecommendation(
                    ...
                    focus_accounts=sorted(set(rule.focus_accounts + explanation["audit_focus"])),
                    focus_processes=rule.focus_processes,
                    recommended_procedures=sorted(set(rule.recommended_procedures + explanation["procedures"])),
                    evidence_types=rule.evidence_types,
                    recommendation_text=f"{rule.name}：建议优先关注{'、'.join(rule.focus_accounts)}。",
                )
            )
            db.add(
                RiskAlertRecord(
        ...
        db.commit()
        return {"run_id": run.id, "status": run.status, "summary": run.summary, "results": results}
            ...
            payload.append(
                {
                    ...
                    "focus_accounts": sorted({item for rec in recs for item in rec.focus_accounts}),
                    "focus_processes": sorted({item for rec in recs for item in rec.focus_processes}),
                    "recommended_procedures": sorted({item for rec in recs for item in rec.recommended_procedures}),
                    "evidence_types": sorted({item for rec in recs for item in rec.evidence_types}),
                }
            )
        return payload

    def _run_anomaly_detection(self, enterprise_id: int, run_id: int, features: dict, financials: list) -> RiskIdentificationResult | None:
