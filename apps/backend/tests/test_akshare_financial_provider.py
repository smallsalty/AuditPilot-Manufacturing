...
def test_fetch_financials_uses_ths_abstract_when_analysis_indicator_is_empty(monkeypatch):
    class FakeAkshare:
        ...
        def stock_balance_sheet_by_report_em(symbol: str):
            return pd.DataFrame(
                [
                    {"REPORT_DATE": "2025-03-31", "REPORT_TYPE": "一季报", "FIXED_ASSET": 800.0, "ACCOUNTS_RECE": 100.0, "INVENTORY": 50.0},
                    {"REPORT_DATE": "2025-12-31", "REPORT_TYPE": "年报", "FIXED_ASSET": 900.0, "ACCOUNTS_RECE": 200.0, "INVENTORY": 100.0},
                ]
            )
